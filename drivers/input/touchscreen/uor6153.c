...
//printk(KERN_ERR "no filter: (Z1,Z2)=(%d,%d)!!!\n", z1, z2);
			...
			//if((x<100) || (imapx_gpio_getpin(GPIOPortE_Pin3, IG_NORMAL)))
			if((x<100) || (imapx_gpio_getpin(uor6153_int, IG_NORMAL)))
			{
			nTouch =  ZERO_TOUCH;
				}
...
//	gADPoint.y = 0;
	...
	uor6153_int = __imapx_name_to_gpio(CONFIG_TP_UOR6153_INT);
	if(uor6153_int == IMAPX_GPIO_ERROR) {
		printk(KERN_ERR "failed to get UOR6153_int pin.\n");
		return -1;
	}
	GPIOPortE_Pin3 = imapx_gpio_to_irq(uor6153_int);
	imapx_gpio_setirq(uor6153_int, FILTER_MAX, IG_FALL, 1);
...

{
	...
	adapter = i2c_get_adapter(CONFIG_TP_UOR6153_I2C + 1);
	if (!adapter) {
