...
void ix_gps_hw_init(int en)
{
	...
	tmp = __imapx_name_to_gpio(CONFIG_IG_GPS_ENABLE);

	if(tmp == IMAPX_GPIO_ERROR)
	  printk(KERN_ERR "ix_gps: failed to get RF pin.\n");
	else
	  imapx_gpio_setcfg(tmp, IG_OUTPUT, IG_NMSL);
	...
	if(en)
	{
		 ...
		 */
		imapx_gpio_setcfg(IMAPX_GPE_RANGE(11, 13), IG_CTRL0, IG_NORMAL);

		if(tmp != IMAPX_GPIO_ERROR)
		  imapx_gpio_setpin(tmp, 1, IG_NMSL);
	} else {
		...
		if(tmp != IMAPX_GPIO_ERROR)
		  imapx_gpio_setpin(tmp, 0, IG_NMSL);
	}
}
