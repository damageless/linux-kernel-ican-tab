...
{
	unsigned int iic_reset;

	iic_reset = __raw_readl(rAPBP_RST);
	iic_reset |= (0x1<<3);
	__raw_writel(iic_reset,rAPBP_RST); 
					...
					struct i2c_msg *msg)
{
	...
	while(i2c->CurWriteCount && i2c->MasterArbitration)
	{
		for(i=0;i<write_msg->len;i++)
		{
			...
			if(retry == 0)                                                                                                       
			{                                                                                                                    
				printk("[%s] message sent failed data %x address %x\n",__func__, byte, __raw_readl(i2c->base_reg + rIC_TAR));
				      ...
				      struct i2c_msg *msg)
{
	...
	{
		if(msg_num == 2 && (msg[1].flags == I2C_M_RD) && (msg[0].addr == 0x38))
		{
			i2c->msg = &msg[1];               
			...
			{                                                                                                        
				i2c->state = IIC_CUR_IDLE;                                                                       
				printk("functions[%s] iic0 data transfer timeout slave address is %x\n",__func__,i2c->msg->addr);
			}                                                                                                        
		}else{
			for(i=0;i<msg_num;i++)	
			{
...
//	if(msgs->addr == 0x3c)
//		return ret;
	if(msgs->addr == 0x38 || msgs->addr == 0x39)
		msleep(1);
	else
		udelay(1000);
