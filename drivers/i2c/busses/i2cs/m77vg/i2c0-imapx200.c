...
static int imapx200_i2c_init_register(struct imapx200_i2c *i2c)
{
	...
	//	printk("[IMAPX200_IIC] Initialization registers\n");
		...
		//	printk("imapx200_i2c_init_register");
			//msleep(100);
			//udelay(3000);
		}
		...
		__raw_writel(0xa9, i2c->base_reg + rIC_SS_SCL_HCNT); // 100
		__raw_writel(0xa9, i2c->base_reg + rIC_SS_SCL_LCNT); // 100
				      ...
				      struct i2c_msg *msg)
{
	...
	//if(i2c->state == IIC_CUR_IDLE)
	{
		if(i2c->dirty_bit == 1)
		{
		//	printk("imapx200_i2c_message_start : dirty\r\n");
		...
		else
		{
			if(msg->flags & I2C_M_RD)
			{
				...
				//			printk("imapx200_i2c_message_start : read\r\n");
				imapx200_i2c_read(i2c,msg);
			}
			else
			{
				i2c->state = IIC_CUR_WRITE;
				//			printk("imapx200_i2c_message_start : write\r\n");
...
//	printk("imapx200_i2c_set_master : imapx200_i2c_init_register\r\n");
	imapx200_i2c_init_register(i2c);	
	return -ETIMEDOUT;
}
...
//			printk("%s data read over\n",__func__);
		...
		writeNum = i2c->CurWriteCount > txFifoEmptyNum ? txFifoEmptyNum : i2c->CurWriteCount;
		 ...
		 if(i2c->CurWriteCount <= 0)
		 {
			...
			i2c->msg_idx++;
			i2c->msg_num--;
//			printk("%s  stop  interrupt data write over\n",__func__);
			imapx200_enable_xmitinterrupt(i2c,0);
			wake_up_interruptible(&i2c->wait);
		 }
