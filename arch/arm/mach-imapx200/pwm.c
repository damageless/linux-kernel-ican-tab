...
static uint32_t save_tcfg0, save_tcfg1, save_tcon,
				save_tcntb[4], save_tcmpb[4];
...
int imap_pwm_suspend(struct sys_device *pdev, pm_message_t pm)
{
	...
	for(i = 0; i < 4; i++)
	{
		save_tcntb[i] = readl(ioaddr + IMAP_TCNTB0 + (i * 12));
		save_tcmpb[i] = readl(ioaddr + IMAP_TCMPB0 + (i * 12));
	}
...
int imap_pwm_resume(struct sys_device *pdev)
{
	...
	for(i = 0; i < 4; i++)
	{
		writel(save_tcntb[i], ioaddr + IMAP_TCNTB0 + (i * 12));
		writel(save_tcmpb[i], ioaddr + IMAP_TCMPB0 + (i * 12));
	}
...
#ifdef CONFIG_IG_BL_PWM_MUX
	i = CONFIG_IG_BL_PWM_MUX;
#endif

#ifdef CONFIG_IG_BL_PWM_PRE
	j = CONFIG_IG_BL_PWM_PRE;
#endif
