...
void imap_mem_reserve(void)
{
	...
	/* added by sololz */
	/* 
	 * read information 1 register whitch might be set by user to be a 
	 * more reserved memory mark. if this mark is set here memalloc 
	 * reserved memory size will be set to be 96MB(by default). this 
	 * takes effect supposes that user restart system using soft restart.
	 * user process will write information 1 register, this register will
	 * only be reset at power on. if kernel runs here, memory structure 
	 * has already formed, so register map to high memory is feasible.
	 */
	while(1)
	{
		unsigned int *info1_reg = (unsigned int *)(0xf0200230);

		if(info1_reg[0] == 0x64020128)
		{
			/* being discussed, reserved memory should never be larger than 128MB */
			if((CONFIG_IMAP_MEMALLOC_MAX_RESERVE_SIZE >= CONFIG_IMAP_MEMALLOC_SYSTEM_RESERVE_SIZE) && \
					(CONFIG_IMAP_MEMALLOC_MAX_RESERVE_SIZE <= 128))
				reservemem_devs[RESERVEMEM_DEV_MEMALLOC].size = CONFIG_IMAP_MEMALLOC_MAX_RESERVE_SIZE * SZ_1M;
			else
			{
				/* do nothing */
			}
