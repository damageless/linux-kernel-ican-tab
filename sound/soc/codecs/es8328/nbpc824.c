...
struct work_struct exchange_i2c;
/*
 * Debug
 */
...
static void externel_record(struct snd_soc_codec *codec)
{
	...
	es8328_write(codec, ES8328_ADCCONTROL14, 0xcb); //0x16
}

static void internel_record(struct snd_soc_codec *codec)
{
	...
	es8328_write(codec, ES8328_ADCCONTROL11, 0xe0); //0x13
...
//	es8328_write(codec, ES8328_ADCCONTROL14, 0xc1); //0x16
	...
	if (tmp & (1<<15)) {
		internel_record(codec);
	} else {
		externel_record(codec);
	}
	es8328_write(codec, ES8328_CHIPPOWER, 0x00); //0x02
...
static void change_the_codec_register(struct work_struct work)
{
	int tmp;
	struct snd_soc_codec *codec = es8328_socdev->card->codec;
	es8328_write(codec, ES8328_ADCCONTROL7,  0xf4);        
	tmp = __raw_readl(rGPEDAT);                   
	if (tmp & (1<<15)) {
		internel_record(codec);
	} else {
...
static irqreturn_t insert_the_earphone(int irq, void *dev_id)
{
	int tmp;

	tmp = __raw_readl(rEINTG4PEND);
	printk("............tmp is fkjs %d\n",(tmp & (1<<15)));	
	if (tmp & (1<<15)) {
		tmp = __raw_readl(rEINTG4MASK);
		tmp |= (1<<15);
		__raw_writel(tmp, rEINTG4MASK);
		__raw_writel((1<<15), rEINTG4PEND);
		
		schedule_work(&exchange_i2c);	
		tmp = __raw_readl(rEINTG4MASK);
		tmp &= ~(1<<15);
		__raw_writel(tmp, rEINTG4MASK);
...
static int es8328_probe(struct platform_device *pdev)
{
	...
	rGPE = __raw_readl(rGPECON);
	rGPE &= ~(0x3<<30);
	__raw_writel(rGPE, rGPECON);

	rGPE = __raw_readl(rEINTGCON);
	rGPE &= ~(7<<12);
	rGPE |= (7<<12);
	__raw_writel(rGPE, rEINTGCON);
	rGPE = __raw_readl(rEINTGFLTCON0);
	rGPE |= (0xff << 24);
	__raw_writel(rGPE,rEINTGFLTCON0);
	rGPE = __raw_readl(rEINTG4MASK);
	rGPE &= ~(1<<15);
	__raw_writel(rGPE,rEINTG4MASK);
	...
	INIT_WORK(&exchange_i2c, change_the_codec_register);
//#if defined (CONFIG_I2C) || defined (CONFIG_I2C_MODULE)
...
//#endif
		rGPE = request_irq(IRQ_GPIO, insert_the_earphone,
				IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING | IRQF_DISABLED,	
				"es8328", NULL);
		if (rGPE < 0) {
			printk("unable to request irq %d by es8328 ,the error is %d\n", IRQ_GPIO, rGPE);
