...
static int imapx200_probe(struct platform_device *pdev)
{
	...
	tmp = __imapx_name_to_gpio(CONFIG_CODEC_WM8987_POWER);

	if(tmp == IMAPX_GPIO_ERROR)
	  printk(KERN_ERR "get wm8987 power pin failed. may be not configurated.\n");
	else
	{
...
static struct wm8987_setup_data imapx200_wm8987_setup = {
	.i2c_address = 0x1a,
	.i2c_bus = CONFIG_CODEC_WM8987_I2C + 1,
};
